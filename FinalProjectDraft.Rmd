...
As Fig. 1 below shows, the number of seedlings for each of the four species is in a similar range.

```{r}
ggplot(data = trees, mapping = aes(x = Species)) + 
  geom_bar(fill = "lightblue") +
  labs(y = "Count", x = "Tree species",
       title = "Figure 1. Number of trees",
...
# the code above to change levels of Light_Cat was cited from https://community.rstudio.com/t/how-to-manually-order-x-axis-on-bar-chart/9601

ggplot(data = tree_EDA2, 
       mapping = aes(x = Light_Cat, fill = Species)) +
geom_bar() + 
labs(y = "Count", x = "Light level",
     title = "Figure 3. Light levels",
     subtitle = "Segmented by species")
```

Lastly, Fig.4 through Fig. 6 below show the distribution of dates on which trees were planted. Fig. 4 indicates that every tree in the dataset was planted in 2018, Fig. 5 demonstrates that every tree was planted either in May or June, and Fig. 6 reveals that there is more variation in plant day than plant year and month. 

```{r, warning = F, message = F}
# Convert to date obaject. Code cited from https://epirhandbook.com/en/working-with-dates.html
tree_EDA3 <- trees %>% 
...
ggplot(data = tree_EDA3,mapping = aes(x = factor(PlantDateYear))) + 
  geom_bar()+
  labs(title = "Figure 4. Year of Plant Day",
...
ggplot(data = tree_EDA3,mapping = aes(x = PlantDateMonth)) + 
  geom_bar()+
  labs(title = "Figure 5. Month of Plant Date",
       subtitle = "Faceted by species",
       x = "Month", y =  "Count")+
  facet_grid(. ~ Species)
...
ggplot(data = tree_EDA3,mapping = aes(x = PlantDateDay)) + 
  geom_histogram()+
  labs(title = "Figure 6. Day of Plant Date",
       subtitle = "Faceted by species",
       x = "Day", y =  "Count")+
  facet_grid(. ~ Species)
```
...
Before building statistical models for predicting tree survival, we conducted hierarchical clustering to identify any outliers. We then used Principal Component Analysis (PCA) because there were ten numerical variables that could influence the survival of seedlings and using PCA would allow us to reduce the dimension to a smaller number of variables that summarized most of the variability in those variables. Following PCA, we built a random forest model based on loading scores for the principal components to answer one of our research questions which was to compare the accuracy of predictive algorithms for tree survival. 

Although using principal components for random forest is certainly one way of building a predictive model for tree survival, it has limitations. One limitation is that we can only determine variable importance in terms of principal components and not in terms of variables in the original dataset. Another limitation is that we can only use numerical variables for PCA. Given that one of our research questions was to understand what variables in the dataset were more influential in tree survival than others, these limitations had to be avoided. Hence, we built a Cox proportional hazards regression model (Coxph), a support vector machine (SVM) model, and a random forest model by using all the variables (both numerical and categorical) that could influence tree survival as predictors. 
...
As shown in the dendrogram in Fig. 7, there was no split with only one terminal node, which suggested that there was no outlier to be removed for data analysis. Thus, we did not remove any variables in the remaining part of this report. 

### Ai: 4.2 Principal Component Analysis (PCA)

In this section, we calculated principal components of numerical variables that may have impacted the survival of seedlings.
...
* No: This was a unique ID attached to each seedling, thus, it should not have influenced tree survival. 
...
* Time: Time was the status of each seedling at a given time and hence should not have influenced tree survival. 
* Census: This was the number of week that a given tree died or the experiment ended (whichever was earlier); it indicated essentially the same thing as the variable Time. Thus, we removed Census as well.
...
The original dataset had N/A’s for the variable EMF for all observations that had AMF (arbuscular mycorrhizal fungi) in the variable Myco. We contacted Dr. Wood (one of the authors of the article this data was from), and she mentioned that she and other authors did not measure EMF for these observations because the seedlings that have AMF in the variable Myco do not get colonized by EMF (ectomycorrhizal fungi). Hence, we replaced the N/A’s with 0. 

#### Find loading score 
```{r}
# Convert all NA EMF values to 0. Code cited from https://stackoverflow.com/questions/8161836/how-do-i-replace-na-values-with-zeros-in-an-r-dataframe
tree_pca <- trees %>%
...
The above result indicates that the Harrel’s concordance index (c-index) for this random forest model based on loading scores for the principal components is approximately 0.699 ($\approx 1-0.3008536$). This means that, given two random seedlings from the dataset, the model can predict survival with 69.9% accuracy. 


## Andy: 4.4 Survival function and Coxph

After exploratory data analysis, we can use the `Surv` function on the dataset to create a survival object to be used in various functions. We supply the time and event variables (Time and Event respectively) for survival analysis.
...
The Cox proportional hazards model is also possible to implement using a survival object. Using variables which have been selected to present a signficant p-value. 

Outline of section:

Relevance of variables can be judged by placing them in the model and observing whether the p-value for the variable (or any of its factors if it is a character variable) is significant (≤0.05). 

Relevant chr variables include Species, Conspecific, SoilMyco, Sterile.
Relevant numerical variables include Phenolics.
...
## 4.5 Andy: SVM with survival object
...
We can instead use the `survivalsvm` package to predict survival time and receive a concordance index value to compare with other models. When predicting using a training subset and a testing subset, the C-index value for survivalsvm is quite high.
...
svm_tree <- survivalsvm(Surv(Time, Event)~.-c(PlantDateYear, Census), data=train, type="vanbelle1", diff.meth="makediff1", gamma.mu=1)
...
## 4.6 Ellie: Single Survival Tree
```{r}
...
```
The above histograms (Fig. 6 and Fig. 7) portray the dataset's distribution of lignin and phenolics content across trees in the dataet. Both appear to have two groupings with higher and lower content in each of the two variables. 
