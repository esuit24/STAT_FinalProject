...
# to change levels of Light_Cat in the code above, we refereed to this website: https://community.rstudio.com/t/how-to-manually-order-x-axis-on-bar-chart/9601
...
tree_distances1 <- dist(tree_scaled)
...
As shown in the dendrogram in Fig. 7, there was no split with only one terminal node, which suggested that there was no outlier to be removed for data analysis. Thus, we did not remove any variables in the remaining sections of this report. 
...
tree_pca <- replace(trees, is.na(trees$EMF), 0.00)
...
tree_pca <- tree_pca %>%
  ...
  mutate(PlantDateYear = year(PlantDate),
         PlantDateMonth = month(PlantDate),
         PlantDateDay = day(PlantDate))  %>%
  select(-PlantDate, -PlantDateYear)
...
The above result indicates that the Harrel’s concordance index (c-index) for this random forest model based on loading scores for the principal components is approximately 0.699. This means that, given two random seedlings from the dataset, the model can predict survival with 69.9% accuracy. 
